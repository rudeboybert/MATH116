...
Say we'd like to `_join` i.e. merge together the two into a single data frame.
We join by `"x1"`. Note how it is in quotation marks.
...
* There are many types of `_join` (right-hand column of back of cheatsheet)
* To keep things simple, we'll only use
    + `left_join`
    + `full_join`
* This
