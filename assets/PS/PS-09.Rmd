...
simulations <- do(10) * mean(final ~ shuffle(even), data=grades)


ggplot(data=permuted_diff_means, aes(x=diff)) +
  geom_histogram() +
...
permuted_diff_means <- do(10) * mean(final ~ shuffle(even), data=grades)
permuted_diff_means <- permuted_diff_means %>% 
  as_data_frame()
