...
**Solution:** The key difficulty of this question was distiguising between using
`n()` to count observations vs `sum()`. After `View(Titanic)` we see that for
each `Class`, `Sex`, `Age`, `Survived` category we report the count in the
variable `Freq`. We need to `sum(Freq)`
