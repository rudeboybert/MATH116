...
Just switch `month` above with `origin`. First with `%>%` piping and without. Recall
`%>%` is pronounced "then":
...
And without. We this is a little more awkward, as we first need to create an intermediate 
variable `weather_group_by_airport`. I would argue the above approach is much cleaner.
