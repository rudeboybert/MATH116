...
1. Come up with an airline industry explanation on why the arrival delays for Alaska and Frontier may differ.
...
Note: We are only investigating flights where the delay was not extreme (less
than 250min). Let's look at the regression output:
...
It appears that Frontier is on average 17min later than Alaska. Why? Let's dig
deeper:
...
med <- median(alaska_frontier_flights$temp, na.rm = TRUE)



alaska_frontier_flights <- alaska_frontier_flights %>% 
  mutate(high_temp = ifelse(temp > med, "Above Median Temp of 57F/13C", "Below Median Temp of 57F/13C")) %>% 
  filter(!is.na(high_temp)) 

means <- alaska_frontier_flights %>% 
  group_by(carrier, high_temp) %>% 
  summarise(mean=mean(arr_delay, na.rm=TRUE))

means2 <- alaska_frontier_flights %>% 
  group_by(high_temp) %>% 
  summarise(mean=mean(arr_delay, na.rm=TRUE))

ggplot(alaska_frontier_flights, aes(col=carrier)) +
  geom_point(aes(x=dep_delay, y=arr_delay)) +
  geom_hline(data=means, aes(col=carrier, yintercept=mean)) +
  geom_hline(data=means2, aes(yintercept=mean), linetype="dashed") +
    facet_wrap(~high_temp)
```
...
United Airlines is in the news a lot lately. Do they have bigger departure
delays than their rivals? 
...
1. Create a visualization that illustrates the difference in departure delays
between UA and AA. Which initially appears to have larger delays?
1. What are the mean departure delays for both airlines?
1. Run a linear regression with
    + $y$ = departure delay (continuous/numerical variable)
    + $x$ = carrier (categorical variable with two levels)
1. Compare the two means in Step 3 with the regression results.


## Discussion

Remember, to pick out UA and AA flights, you need the OR operator: for all rows
...
As you now know, I'm a big fan of boxplots: comparisons across groups with a
single line! In this case, the median delay for United appears to be higher!
...
The mean departure delays below. We see that United is on average 15.665 -
15.057 = 0.608 minutes later in their departures.
...
$\beta_{\mbox{carrierUA}}$ is 0.608! i.e. United has on average 0.608 minute
bigger delays, just like we computed above! However notice:

* The 95% confidence interval [-8.510, 9.726] includes 0! In other words, a
differential of 0 is still plausible!
* The p-value is very high! In other words, we would not reject $H_0$ i.e. there
may still be a true difference in delays between United and American, but this
data on January 31st doesn't suppor this claim.
